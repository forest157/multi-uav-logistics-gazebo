...
三架无人机均挂载 16 线 3D 激光雷达，点云统一转换到 world 坐标系；实验检测链路包含地面与机体剔除、体素背景学习、欧式聚类、跨帧关联和三帧确认。RViz 可同时查看融合点云、确认目标和速度预测，上位机可选择稳定仿真感知、物理 3D 雷达实验感知或 Gazebo 真值对照，并显示背景学习、候选簇和确认目标数量。v0.5.1 进一步区分“新鲜空检测”和“感知断流”：无障碍时保持 SAFE，真正超时才安全悬停；同时按鸟类真实尺寸过滤建筑碎片，并在上位机显示原始簇、过滤数、候选数和确认数。物理雷达仍属实验输入，默认继续使用稳定仿真感知。
