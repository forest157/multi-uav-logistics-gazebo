...
class OperatorPlugin(Plugin):
    def __init__(self, context):
        ...
        title=QLabel("三机物流任务控制台");title.setObjectName("title");subtitle=QLabel("场景预览 · 地图选点 · 低空避障 · 编队往返");subtitle.setObjectName("subtitle");root.addWidget(title);root.addWidget(subtitle)
        style_path=os.path.join(rospkg.RosPack().get_path("logistics_gazebo_sim"),"config","operator.qss")
    ...
    def planning_error(self,text):
        mapping={"E_ALTITUDE":"\u98de\u884c\u9ad8\u5ea6\u5fc5\u987b\u5728 3\uff5e45 m","E_BOUNDARY":"\u8d77\u70b9\u6216\u7ec8\u70b9\u8d85\u51fa\u5b89\u5168\u8fb9\u754c","E_BLOCKED":"\u8d77\u70b9\u6216\u7ec8\u70b9\u4f4d\u4e8e\u969c\u788d\u7269\u5b89\u5168\u533a\u5185\uff0c\u8bf7\u8c03\u6574\u5750\u6807\u6216\u9ad8\u5ea6","E_DISTANCE":"\u8d77\u70b9\u4e0e\u7ec8\u70b9\u8ddd\u79bb\u5fc5\u987b\u81f3\u5c11\u4e3a 3 m","E_NO_PATH":"\u5f53\u524d\u9ad8\u5ea6\u4e0e\u961f\u5f62\u4e0b\u89c4\u5212\u4e0d\u51fa\u5b89\u5168\u8def\u5f84\uff0c\u8bf7\u63d0\u9ad8\u9ad8\u5ea6\u6216\u8c03\u6574\u8d77\u7ec8\u70b9","E_SCENE":"\u573a\u666f\u7f16\u53f7\u65e0\u6548","E_TOPPRA":"B\u6837\u6761\u6216 TOPPRA \u65f6\u95f4\u53c2\u6570\u5316\u5931\u8d25\uff0c\u8bf7\u8c03\u6574\u8d77\u7ec8\u70b9\u6216\u9ad8\u5ea6"}
    ...
    def launch_sim(self):
        ...
        sid=self.scene.currentData();pkg=rospkg.RosPack().get_path("logistics_gazebo_sim");mission="/tmp/logistics_custom_mission_{}.yaml".format(os.getpid())
